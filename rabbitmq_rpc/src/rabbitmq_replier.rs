...
pub type OnRequestCallback =
    Arc<dyn Fn(Vec<u8>) -> BoxFuture<'static, Result<Vec<u8>>> + Send + Sync>;
...
impl amqprs::consumer::AsyncConsumer for MyAsyncRequestConsumer {
    async fn consume(
        ...
        let properties = amqprs::BasicProperties::default()
            ...
            .finish();
        let result = channel
            .basic_publish(properties, response_content_bytes, args)
            .await;
        if result.is_err() {
            println!("{:?}", result.err());
        }
...
impl QueueRequestConsumer {
    ...
    pub async fn start_consuming(&self, channel: &Channel) -> Result<()> {
        let request_consumer_args = amqprs::channel::BasicConsumeArguments::new(
            &self.request_queue_name,
            &self.request_consumer_tag,
        )
        .no_ack(true)
        .finish();

...
impl QueueReplyConsumer {
  ...
  pub async fn request(&self, channel: &Channel, message_type: &str, request_bytes: Vec<u8>, timeout_ms: u64) -> Result<Vec<u8>> {
    ...
    let properties = amqprs::BasicProperties::default()
      ...
      .finish();
    channel.basic_publish(properties, request_bytes, args).await?;
  ...
  pub async fn start_consuming(&self, channel: &Channel) -> Result<()> {
    let reply_consumer_args = amqprs::channel::BasicConsumeArguments::new(&self.reply_queue_name, &self.reply_consumer_tag)
      .no_ack(true)
      .finish();
